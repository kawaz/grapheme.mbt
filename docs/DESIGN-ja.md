...
```moonbit
// InCB=Linker のコードポイント（3バイト/エントリ、ビッグエンディアン、バイナリサーチ）
let incb_linker_packed : Bytes = b"..."

// InCB=Extend のコードポイント/レンジ（6バイト/エントリ: start 3B + end 3B、ビッグエンディアン）
...
- `incb_linker_packed`: InCB=Linker のコードポイントテーブル（3バイトパック）
- `incb_extend_packed`: InCB=Extend のレンジテーブル（6バイトパック）
...
fn is_incb_linker(cp : Int) -> Bool {
  // incb_linker_packed に対するバイナリサーチ（3バイトパック）
}

/// コードポイントが InCB=Extend かどうかを返す。
fn is_incb_extend(cp : Int) -> Bool {
  // incb_extend_packed に対するバイナリサーチ（6バイトパック）
}
```
